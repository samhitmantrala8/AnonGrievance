...
export const postMessage = async (req, res) => {
  try {
    ...
    if (!username || !description) {
      return res
        .status(400)
        .json({ message: "Username and description are required" });
    }
...
export const getMessages = async (req, res) => {
  try {
    const messages = await Message.find()
      .select("username description media upvotes downvotes comments createdAt")
      .sort({ createdAt: -1 });
    res.json(messages);
  } catch (error) {
...
export const deleteMessage = async (req, res) => {
  try {
    ...
    // Assuming `req.user` is populated with the authenticated user's info
    if (!req.user) {
...
export const addUpvote = async (req, res) => {
  try {
    ...
    if (hasUpvoted) {
      ...
      if (hasDownvoted) {
        message.downvotes = message.downvotes.filter(
          (user) => user !== username
        );
      }
    ...
    res
      .status(500)
      .json({ message: "Failed to toggle upvote", error: error.message });
  }
};
...
export const addDownvote = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to toggle downvote", error: error.message });
  }
};
